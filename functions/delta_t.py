...
functions:
    ...
    * plot_grid_mult - function for plotting a grid of NxN plots (N for
...
def plot_grid_mult_cut(
    ...
    with multiple .dat files, keeping the data only for the required pixels reducing
    the memory occupied.
    ...
    same_y : bool, optional
        Switch for equalizing the y axis for all plots. The default is True.
    ...
    for q in tqdm(range(len(pix)), desc="Minuend pixel   "):
        for w in tqdm(range(len(pix)), desc="Subtrahend pixel"):
            ...
            try:
                # n_max = np.argmax(n)
                peak_max = np.max(n).astype(np.intc)
                # arg_max = format((bins[n_max] + bins[n_max + 1]) / 2, ".2f")
            except Exception:
                # arg_max = None
                peak_max = None
