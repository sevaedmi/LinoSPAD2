...
for q in range(5):
    for w in range(5):
        ...
        axs[q][w-1].set_xlabel('Time bin [-]')
        axs[q][w-1].set_ylabel('\u0394t [ps]')
        axs[q][w-1].hist2d(time_bin, output, bins=(200, 200))
        ...
        try:
            os.chdir("results/delta_t vs time")
        except Exception:
            os.mkdir("results/delta_t vs time")
            os.chdir("results/delta_t vs time")
        fig.tight_layout()  # for perfect spacing between the plots
        plt.savefig("{name}_delta_t_time_grid.png".format(name=filename))
        os.chdir("../..")
