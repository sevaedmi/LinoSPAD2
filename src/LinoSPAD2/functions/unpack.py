...
def unpack_bin(
    ...
    FileNotFoundError
        Controller for stopping the script in the case of no calibration
        data were found.
...
def unpack_bin_wat(
    ...
    """Unpack data from firmware version 2212 with absolute timestamps.

    Unpacks binary-encoded data from LinoSPAD2 firmware version 2212.
    Uses Numpy to achieve the best speed for unpacking. Data is returned
    as a 3D array where rows are TDC numbers, columns are the data, and
    each cell contains a pixel number in the TDC (from 0 to 3) and the
    timestamp recorded by that pixel. Additionally, an array of
    absolute timestamps is returned as well.
    ...
    data_all : array-like
        3D array of pixel coordinates in the TDC and the timestamps.
    data_at_conv : array-like
        Array of absolute timestamps in ps, with length equal to
    ...
    rawFile = np.fromfile(file, dtype=np.uint32)
    # timestamps are lower 28 bits
    data_raw = (rawFile & 0xFFFFFFF).astype(np.longlong)
    ...
    data_at_conv = np.zeros(cycles)

    # Collect the absolute timestamps indices
    for cyc in range(cycles):
        ind.extend(
            [x for x in range(cyc * (65 * 300 + 2), cyc * (65 * 300 + 2) + 2)]
        )

    # Absolute timestamps only
    data_at = data_raw[ind].reshape(cycles, 2)

    # Convert the absolute timestamps from binary to decimal (ps)
    for cyc in range(cycles):
        data_at_conv[cyc] = int(
            "0b"
            + bin(int(data_at[cyc][1]))[2:]
            + bin(int(data_at[cyc][0]))[2:],
            2,
        )

    # Standard data: everything besides the absolute timestamps
    data_st = np.delete(data_raw, ind)

    # pix adress in the given TDC is 2 bits above timestamp
    data_p = ((data_st >> 28) & 0x3).astype(np.longlong)
    data_st[np.where(data_st < 0x80000000)] = -1
    # number of acquisition cycle in each datafile
    ...
    data_matrix_t = (
        data_st.reshape(cycles, 65, timestamps)
        .transpose((1, 0, 2))
        .reshape(65, timestamps * cycles)
    )
    # cut the 65th TDC that does not hold any actual data from pixels
